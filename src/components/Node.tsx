import React from "react";
import ExpandMoreIcon from "@mui/icons-material/ExpandMore";
import {
  Accordion,
  AccordionSummary,
  Typography,
  AccordionDetails,
  Box,
} from "@mui/material";
import { styled } from "@mui/material/styles";
import colors from "../constants/colors";
import Status from "./Status";
import { Node as NodeType } from "../types/Node";
import { Block } from "../types/Block";
import { getBlocks } from "../reducers/blocks";

type Props = {
  node: NodeType;
  expanded: boolean;
  toggleNodeExpanded: (node: NodeType) => void;
  blocks: Block[];
};

const AccordionRoot = styled(Accordion)({
  margin: "16px 0",
  boxShadow: "0px 3px 6px 1px rgba(0,0,0,0.15)",

  "&:before": {
    backgroundColor: "unset",
  },
});

const AccordionSummaryContainer = styled(AccordionSummary)({
  padding: "0 24px",
  "& .MuiAccordionSummary-content": {
    margin: "10px 0 !important", // Avoid change of sizing on expanded
  },
  "& .MuiAccordionSummary-expandIconWrapper": {
    color: colors.faded,
  },
});

const BoxSummaryContent = styled(Box)({
  display: "flex",
  flexDirection: "row",
  justifyContent: "space-between",
  alignItems: "center",
  width: "100%",
  paddingRight: 20,
});

const TypographyHeading = styled(Typography)({
  fontSize: 17,
  display: "block",
  color: colors.text,
  lineHeight: 1.5,
});

const TypographySecondaryHeading = styled(Typography)(({ theme }) => ({
  fontSize: 14,
  color: colors.faded,
  lineHeight: 2,
}));

const ContentBlock = styled(Typography)(({ theme }) => ({
  background: colors.backgroundBlock,
  padding: "8px",
  borderRadius: "2px",
  margin: "4px",
}));

const ContainerBlock = styled(Typography)({});

const IndexBlock = styled(Typography)({
  fonteWeight: "bold",
  fontSize: "10px",
  lineHeight: "16px",
  color: colors.textBlock,
});

const TextBlock = styled(Typography)({
  fontSize: "14px",
  lineHeight: "20px",
  color: colors.text,
  letterSpacing: "0.25px",
});

const Node: React.FC<Props> = ({
  node,
  expanded,
  toggleNodeExpanded,
  blocks,
}) => {
  function getBlocks() {
    const status = blocks[0]?.status || "ok";
    if (status === "loading" || status === "error") {
      return <Typography>{status}</Typography>;
    }
    if (blocks.length > 0) {
      return (
        <ContainerBlock>
          {blocks.map((block) => {
            return (
              <ContentBlock>
                <IndexBlock>{block.attributes?.index}</IndexBlock>
                <TextBlock>{block.attributes?.data}</TextBlock>
              </ContentBlock>
            );
          })}
        </ContainerBlock>
      );
    }
    return <Typography>Empty</Typography>;
  }

  return (
    <AccordionRoot
      elevation={3}
      expanded={expanded}
      onChange={() => toggleNodeExpanded(node)}
    >
      <AccordionSummaryContainer expandIcon={<ExpandMoreIcon />}>
        <BoxSummaryContent>
          <Box>
            <TypographyHeading variant="h5">
              {node.name || "Unknown"}
            </TypographyHeading>
            <TypographySecondaryHeading variant="subtitle1">
              {node.url}
            </TypographySecondaryHeading>
          </Box>
          <Status loading={node.loading} online={node.online} />
        </BoxSummaryContent>
      </AccordionSummaryContainer>
      <AccordionDetails>{getBlocks()}</AccordionDetails>
    </AccordionRoot>
  );
};

export default Node;
